...
<head>
	<meta charset="UTF-8">
	<title>Iniciar sesion empleado</title>
</head>
<body>
	<div class="container mt-5">
		<div class="card">
			<div class="card-header bg-success text-white">
				<h1 class="text-center">Iniciar Sesión Empleado</h1>
			</div>
			<div class="card-body">
				<?= form_open('empleado/proLogin', ['id'=> 'formIni', 'class'=> 'was-validated']); ?>
		...
		$('#formIni').on('submit', function(event) {
			...
			postAjax($(this).attr('action'), data, function(response){
				if (response.success != undefined) {
					...
					setInterval(function(){ window.location = '<?= site_url()."/empleado/index" ?>';}, 3000);
				} else {
